...
-- Check all subjects
SELECT * FROM subjects ORDER BY semester, subject_code;
-- Check subject count by semester
SELECT semester, COUNT(*) as subject_count, SUM(credits) as total_credits 
FROM subjects 
GROUP BY semester;
-- Check if student_subjects table is working (after some enrollments)
SELECT s.subject_code, s.subject_name, ss.professor_name, ss.schedule
